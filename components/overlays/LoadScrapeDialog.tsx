...
import { returnInputElementValue, returnInputElementChecked } from "@utils/generalFunctions";
import { FormEvent, MouseEvent } from "react";

const LoadScrapeDialog = ({load} : {
  ...
  const submit = async (e : MouseEvent<HTMLButtonElement, any> | FormEvent) => { 
    ...
    await load({
      id: returnInputElementValue({elementId: "scraper-id"}), 
      resultsNeeded: !returnInputElementChecked({elementId:  "scraper-results"}), 
      confirmNeeded: true
    });
...
export default LoadScrapeDialog;